...
struct CourierAuthSessionManagerTests {
    @Test
    func signInThenRefreshUsesStoredRefreshToken() async throws {
        ...
        var manager = CourierAuthSessionManager(client: client)
        ...
        #expect(recorder.paths == ["/app/v1/courier/session/exchange", "/app/v1/courier/session/refresh"])
    ...
    func refreshWithoutSignInThrows() async {
        ...
        } catch CourierAuthSessionManagerError.missingSession {
            // Expected path.
        } catch {
