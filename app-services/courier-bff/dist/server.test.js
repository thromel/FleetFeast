...
function createTestCourierDependencies(listAvailableJobs, getFeatureFlagSnapshot = async () => ({
    flags: {
        "courier.offlineReplay": true,
    },
    ttlSeconds: 30,
    generatedAtEpochMillis: 1_735_680_900_000,
}), acceptJob = async (jobId, courierId) => ({
    jobId,
    orderId: jobId,
    status: "ACCEPTED",
    courierId,
}), pickupJob = async (jobId, courierId) => ({
    jobId,
    orderId: jobId,
    status: "PICKED_UP",
    courierId,
}), dropoffJob = async (jobId, courierId) => ({
    jobId,
    orderId: jobId,
    status: "DROPPED_OFF",
    courierId,
...
test("courier-bff refresh endpoint rotates refresh token", async () => {
    ...
    try {
        ...
        const exchange = await fetch(`http://127.0.0.1:${address.port}/app/v1/courier/session/exchange`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
                oidcToken: "dev:courier-user-1:courier-1@fleetfeast.dev:courier",
...
test("courier-bff returns feature-flag snapshot", async () => {
    const app = createCourierBffServer(createTestCourierDependencies(async () => [], async () => ({
        flags: {
            "courier.offlineReplay": true,
    ...
    try {
        ...
        const response = await fetch(`http://127.0.0.1:${address.port}/app/v1/courier/feature-flags?userId=courier-1&role=courier&tenantId=fleet-1`);
        assert.equal(response.status, 200);
        const payload = (await response.json());
