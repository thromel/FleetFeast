...
function createTestConsumerDependencies(
  getOrderById: (orderId: string) => Promise<{ id: string; status: string; timelineVersion: number }>,
  ...
  quickCreateOrder: () => Promise<{ id: string; status: string; timelineVersion: number }> = async () => ({
...
test("consumer-bff exchanges OIDC token into app session and token pair", async () => {
  const app = createConsumerBffServer(
    createTestConsumerDependencies(async () => ({
      id: "order-1",
  ...
  try {
    ...
    const payload = (await response.json()) as {
      session: { persona: string; role: string; userId: string; refreshTokenId: string };
      tokenPair: { accessToken: string; refreshToken: string; refreshExpiresAt: string };
    };
...
test("consumer-bff refresh endpoint rotates refresh token and rejects replay", async () => {
  const app = createConsumerBffServer(
    createTestConsumerDependencies(async (orderId: string) => ({
      id: orderId,
  ...
  try {
    ...
    const replayPayload = (await replay.json()) as { errorCode: string };
...
test("consumer-bff returns feature-flag snapshot", async () => {
  const app = createConsumerBffServer(
    createTestConsumerDependencies(
  ...
  try {
    ...
    const response = await fetch(
      `http://127.0.0.1:${address.port}/app/v1/consumer/feature-flags?userId=consumer-1&role=consumer&tenantId=metro-1`,
    );
    assert.equal(response.status, 200);
