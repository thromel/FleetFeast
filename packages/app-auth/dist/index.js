...
export function mapAppAuthError(error) {
...
export function createJwksOidcVerifier(options) {
...
export function createOidcVerifierFromEnv(env = process.env) {
    ...
    if (!jwksUri || !issuer || !audience) {
        throw new AppAuthError("OIDC_VERIFIER_MISCONFIGURED", "OIDC_JWKS_URI, OIDC_ISSUER, and OIDC_AUDIENCE must be provided together");
    }
...
export class InMemoryRefreshSessionStore {
    byHash = new Map();
    hashesByFamily = new Map();
    async put(record) {
        this.byHash.set(record.tokenHash, { ...record });
        const familyHashes = this.hashesByFamily.get(record.familyId) ?? new Set();
...
export class AppSessionAuthService {
    ...
    async refreshSession(input) {
        ...
        if (record.deviceId && record.deviceId !== input.deviceId) {
            throw new AppAuthError("APP_REFRESH_TOKEN_DEVICE_MISMATCH", "Refresh token does not match bound device");
        }
        if (record.usedAt) {
    ...
    async issueSessionInternal(input) {
    ...
    async signAccessToken(session, deviceId) {
...
export function createAppSessionAuthService(options) {
    return new AppSessionAuthService(options);
}
export function createAppSessionAuthServiceFromEnv(env = process.env, options = {}) {
...
function toAppAccessTokenClaims(payload) {
...
function isAppPersona(candidate) {
    return (candidate === "consumer" ||
        candidate === "courier" ||
        candidate === "merchant" ||
        candidate === "admin");
