...
import { AppAuthError, InMemoryRefreshSessionStore, createAppSessionAuthService, createDevOidcVerifier, } from "./index.js";
...
test("refresh requires device match when bound", async () => {
    ...
    await assert.rejects(auth.refreshSession({
        ...
        deviceId: "device-b",
    }), (error) => error instanceof AppAuthError && error.code === "APP_REFRESH_TOKEN_DEVICE_MISMATCH");
});
