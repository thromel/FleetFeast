...
test("internal payment audit route returns append-only audit records", async () => {
  ...
  try {
    ...
    const capture = await fetch(`${baseUrl}/api/v1/payments/capture`, {
      ...
      body: JSON.stringify({
        ...
        idempotencyKey: "audit-route-capture-key",
      }),
    });
    assert.equal(capture.status, 200);

    const audit = await fetch(`${baseUrl}/internal/payments/audit/${order.id}`, {
      method: "GET",
    });
    assert.equal(audit.status, 200);
    const payload = (await audit.json()) as { records: Array<{ actionType: string }> };
    assert.ok(payload.records.some((record) => record.actionType === "PAYMENT_AUTHORIZED"));
    assert.ok(payload.records.some((record) => record.actionType === "PAYMENT_CAPTURED"));
  } finally {
    listener.close();
  }
});
